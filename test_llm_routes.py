...
class LLMRoutesTest(unittest.TestCase):
    ...
    def test_settings_and_ping_do_not_change_board(self):
        ...
        code,result=self.request('POST','/api/llm/settings',{'api_key':'unit-test-placeholder','enabled':False})
        self.assertEqual(code,200);self.assertNotIn('unit-test-placeholder',json.dumps(result))
