...
export function applyAction(state,a,ctx){let s=clone(state);const type=a.type,lessonId=s.lesson?.id,helped=new Set(ctx.helped||[]),runs={...(ctx.runs||{})};let event=null,note=null;
 ...
 if(type==='play'||type==='ai_move'){
  ...
  if(s.demo_active)throw new Error('请先返回原局面。');if(type==='practice_mode'){if(!['recommended','sequential','review'].includes(a.mode))throw new Error('练习模式无效。');runs._practice_mode=a.mode;}const mode=runs._practice_mode||'recommended';let id=type==='retry'?s.lesson?.id:type==='lesson'?a.id:mode==='recommended'?recommend(ctx.catalog,ctx.evidence,ctx.learning,ctx.recent,s.lesson?.id).id:practice({...ctx,runs,helped:[...helped]},s.lesson?.id,mode,type==='next_lesson').next_id;if(!id){const progress=practice({...ctx,runs,helped:[...helped]},s.lesson?.id,mode);s.message=mode==='review'?'当前没有待复习的题目。':progress.book_title?`《${progress.book_title}》已导入的题目全部完成。`:'本轮题目已全部通关。';return {state:s,helped:[...helped],runs,event,note};}const lesson=ctx.catalog.find(l=>l.id===id)||(id==='custom'?s.lesson:null);if(!lesson)throw new Error('找不到这道练习。');s=lessonState(lesson);if(s.lesson.sequence){s._branch_seed=runs[id]||0;runs[id]=(runs[id]||0)+1;}
 }
 ...
 else if(type==='hint'){if(s.mode==='lesson'&&!s.lesson_attempted){s.assisted=true;helped.add(lessonId);}s.message=s.lesson?.hint||'先数整块棋的气。';}
 else if(type==='inspect'){if(!Number.isInteger(a.x)||!Number.isInteger(a.y)||a.x<0||a.y<0||a.x>=s.size||a.y>=s.size)throw new Error('坐标无效。');const g=group(s.board,a.x,a.y);s.inspection={stones:g.stones.map(([x,y])=>({x,y})),liberties:g.liberties.map(([x,y])=>({x,y})),color:s.board[a.y][a.x]};s.marks=g.liberties.map(([x,y])=>({x,y,label:'气'}));if(g.stones.length&&s.mode==='lesson'&&!s.lesson_attempted){s.assisted=true;helped.add(lessonId);}s.message=g.stones.length?`这块棋有${g.stones.length}颗棋子、${g.liberties.length}口气。`:'这里是空点，请选一颗棋子。';}
 else if(type==='feedback'){const text=String(a.text||'').trim();if(!text||text.length>4000)throw new Error('想法需为1至4000字。');note={text,move_number:s.move_number,created_at:new Date().toISOString(),profile_id:ctx.profileId,lesson_id:s.lesson?.id||null};s.message='想法已保存。';}
