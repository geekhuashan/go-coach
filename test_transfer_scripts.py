...
class TransferScriptsTest(unittest.TestCase):
    def setUp(self):
        ...
        self.password=self.root/'password';self.password.write_text('NOT_A_REAL_PASSWORD\n')
        self.requests=[];self.user_agents=[];self.fail_path=None;self.redirect=False;self.mismatch=False;self.omit_profile=False
        self.store={'schema':2,'revision':10,'active_profile_id':'p','profiles':{'p':{'id':'p','name':'测试者','state':{'size':9,'board':[[0]*9 for _ in range(9)],'mode':'lesson','to_play':1,'moves':[]},'attempts':[{'lesson_id':'capture','correct':True,'api_key':'YOUR_API_KEY'}],'notes':[],'helped_lesson_ids':[]}},'matches':{},'llm_settings':{'api_key':'YOUR_API_KEY'},'raw_image':'PHOTO_BYTES_SECRET'}
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def handle_request(self):
                ...
                if outer.fail_path==self.path:
                    self.send_response(409);self.end_headers();self.wfile.write(b'NOT_A_REAL_PASSWORD COOKIE_SECRET YOUR_API_KEY');return
    ...
    def call(self,main,args):
        ...
        for sentinel in ('NOT_A_REAL_PASSWORD','COOKIE_SECRET','YOUR_API_KEY','PHOTO_BYTES_SECRET'):
    ...
    def test_publish_cookie_revision_and_readback(self):
        ...
        self.assertEqual(self.requests[0][2],{'password':'NOT_A_REAL_PASSWORD'})
    ...
    def test_migrate_upload_filters_and_readback(self):
        ...
        self.assertNotIn('PHOTO_BYTES_SECRET',body);self.assertNotIn('YOUR_API_KEY',body)
    ...
    def test_http_remote_and_url_credentials_rejected(self):
        for url in ('http://example.com','https://' + 'example-user' + ':' + 'NOT_A_REAL_PASSWORD' + '@example.com','https://example.com/path','https://example.com?key=secret'):
