...
练习方式可选「智能推荐 / 顺序练习 / 错题复习」，按学习者分别保存。顺序练习按题库顺序推进；家庭导入的书题可按章节进入，并在同一本书内按原题号接续下一章，完成已导入题目后不混入公共题。完整做对即可通关，提示后完成不增加独立答题积分；复习模式保留还没做对的题。19 路局部题默认放大题目所在区域，保留真实坐标和边界提示，可随时切回全盘。走出参考答案后会自动复核：先核对实际提子和作者反驳，再由 KataGo 比较你的落点与参考点。旧待复核局面可点「重新复核」，也可「看参考解法」逐手查看后重练。
