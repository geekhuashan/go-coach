...
def _make_catalog():
    return [
        ...
        _lesson('tactic-net', '枷吃：飞一着套住', 5,
            [(3,2),(2,3),(4,1),(1,4),(5,3),(3,5),(4,2),(4,4)], [(3,3)], [(3,3)], [
                [((5,4),'飞一着套住白棋。这一手先不紧气，而是拦住它逃跑的方向。'),((4,3),'白棋往枷的方向长，整块只剩一口气。'),((3,4),'占掉最后一口气，提掉被套住的白棋。')],
                [((5,4),'飞一着套住白棋。这一手先不紧气，而是拦住它逃跑的方向。'),((3,4),'白棋向另一侧长，仍然出不了这口枷。'),((4,3),'从右侧打吃，白棋只能再向下逃。'),((2,4),'白棋向下长，整块只剩一口气。'),((2,5),'封住最后一口气，提掉套住的白棋。')]],
            '先看白棋还能往哪跑；飞枷是拦住出路，不是立刻紧气。'),
        _lesson('tactic-net-two', '枷吃：两子一起套住', 5,
            [(2,2),(2,3),(3,1),(4,2),(5,3),(3,5),(4,4)], [(3,2),(3,3)], [(3,2),(3,3)], [
                [((5,4),'飞在白棋前方，把两颗白棋一起套进枷里。'),((4,3),'白棋往枷的方向长，整块只剩一口气。'),((3,4),'占掉最后一口气，两颗目标一起提掉。')]],
            '两颗白棋连在一起，也要数整块的气；枷住的是整块，不是其中一颗。'),
        _lesson('tactic-gate', '门吃：先关通道的一端', 5,
            [(1,2),(1,3),(1,4),(3,2),(3,3),(3,4)], [(2,2),(2,3)], [(2,2),(2,3)], [
                [((2,1),'先关上通道的一端，白棋只剩另一头一口气。'),((2,4),'白棋从另一头逃出一格。'),((2,5),'把门彻底关上，提掉通道里的白棋。')]],
            '通道两端都是门。先关一头，再看另一头逃到哪里。'),
        _lesson('tactic-gate-run', '门吃：更长的通道也要关门', 5,
            [(1,2),(1,3),(1,4),(1,5),(3,2),(3,3),(3,4),(3,5)], [(2,2),(2,3),(2,4)], [(2,2),(2,3),(2,4)], [
                [((2,1),'先关上较近的一端，三颗白棋只剩另一头一口气。'),((2,5),'白棋从另一头逃出一格。'),((2,6),'继续关门，提掉通道里的整块白棋。')]],
            '通道更长时，仍然先关一头，不要去通道中间追。'),
        _lesson('tactic-gate-side', '门吃：边上的门也要关上', 5,
            [(1,3),(1,4),(1,2),(1,5)], [(0,3),(0,4)], [(0,3),(0,4)], [
                [((0,2),'先关上边线通道的一端。棋盘边界不会提供气。'),((0,5),'白棋沿边向另一头逃。'),((0,6),'关上另一头，提掉边上的白棋。')]],
            '边线本身就是墙。只要把两端的门关上，白棋就没有气。'),
        _lesson('tactic-hug', '抱吃：从下面抱住两子', 5,
            [(2,2),(3,2),(4,2),(2,3),(5,3),(2,4),(5,4)], [(3,3),(4,3)], [(3,3),(4,3)], [
                [((3,4),'从下面抱住两颗白棋，让它们只剩一口气。'),((4,4),'白棋向下逃出一格。'),((4,5),'继续抱紧，提掉整块白棋。')]],
            '抱吃是从外侧把棋包住，不给它两口气。'),
        _lesson('tactic-hug-two', '抱吃：横块从下边包住', 5,
            [(1,3),(2,2),(3,2),(4,2),(4,3),(1,4)], [(2,3),(3,3)], [(2,3),(3,3)], [
                [((3,4),'从下方抱住横着的两颗白棋。'),((2,4),'白棋向左下逃。'),((2,5),'封住出口，提掉被抱住的白棋。')]],
            '先看整块白棋还剩哪几口气，再选择从哪一侧包住。'),
        _lesson('tactic-wedge', '挖吃：挖进白棋肚子', 5,
            [(2,0),(2,1),(2,2),(2,3),(5,1),(5,2),(5,3),(3,4),(4,4),(4,0),(3,0)], [(3,2),(4,2),(4,3)], [(3,2),(4,2),(4,3)], [
                [((3,3),'挖进白棋形状中间，把气挤掉。'),((3,1),'白棋向上逃。'),((4,1),'占掉最后一口气，提掉整块。')],
                [((3,3),'挖进白棋形状中间，把气挤掉。'),((4,1),'白棋向右上逃。'),((3,1),'占掉最后一口气，提掉整块。')]],
            '挖吃要下在对方棋形内部的要点，而不是在外面追。'),
        _lesson('tactic-snapback-pack', '倒扑：送一子再回提三子', 5,
            [(2,1),(0,2),(1,2),(4,1),(3,2)], [(2,0),(0,1),(1,1),(3,0)], [(0,1),(1,1)], [
                [((1,0),'扑入一子，让白棋来提。'),((0,0),'白棋提掉扑入的黑子，自己整块只剩这个空点。'),((1,0),'在原处回提，提掉这三颗白棋。')]],
            '先算白棋提掉你这一子后还剩几口气；剩一口气就可以回提。'),
        _lesson('tactic-trap-heavy', '接不归：接上另一块也逃不掉', 5,
            [(1,2),(2,1),(1,3),(2,4),(3,4),(4,4),(5,3)], [(2,2),(3,3)], [(2,2)], [
                [((3,2),'打吃上方目标。'),((2,3),'白棋向下连接同伴。'),((4,3),'连接后整块仍只有一口气，全部提掉。')],
                [((3,2),'打吃上方目标。'),((4,3),'白棋向外逃，放弃连接。'),((2,3),'直接提掉目标。')]],
            '接上另一块之前，先数连接后整块还有几口气。'),
    ]
